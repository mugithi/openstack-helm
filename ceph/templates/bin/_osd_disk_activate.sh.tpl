...
function osd_activate {
  ...
  if [ -n "${OSD_JOURNAL}" ]; then
    ...
    if [ -z "${OSD_JOURNAL_PARTITION}" ]; then
      # osd_disk_prepare creates a journal partition if we hand it
      # a raw device.  use that if one isn't specified.
      OSD_JOURNAL_PARTITION=1
    fi
    JDEV=$(dev_part ${OSD_JOURNAL} ${OSD_JOURNAL_PARTITION})
    if [ -d /sys/block/`basename ${OSD_JOURNAL}`/`basename ${JDEV}` ]; then
      OSD_JOURNAL=${JDEV}
    else
      log "ERROR: Unable to find partition ${OSD_JOURNAL_PARTITION} on journal device ${OSD_JOURNAL}"
      exit 1
    fi
